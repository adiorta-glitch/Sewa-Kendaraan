        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-visible">
             <form onSubmit={handleCreateBooking} className="grid grid-cols-1 lg:grid-cols-[1.2fr_2fr] min-h-[600px]">
                ...
                <div className="p-8 space-y-8 overflow-y-auto">
                    ...
                    <div className="pt-6 border-t flex flex-col xl:flex-row gap-3">
                        {editingBookingId && (
                            <div className="flex flex-1 gap-2">
                                 {isSuperAdmin && (
                                    <button type="button" onClick={handleFormDelete} className="flex-1 bg-red-50 text-red-600 py-3 rounded-2xl font-bold uppercase tracking-wider text-[10px] hover:bg-red-100 flex items-center justify-center gap-2 transition-colors">
                                        <Trash2 size={16}/> Hapus
                                    </button>
                                 )}
                                 <button type="button" onClick={handleFormCancelBooking} className="flex-1 bg-orange-50 text-orange-600 py-3 rounded-2xl font-bold uppercase tracking-wider text-[10px] hover:bg-orange-100 flex items-center justify-center gap-2 transition-colors">
                                    <XCircle size={16}/> Batalkan Sewa
                                 </button>
                            </div>
                        )}
                        <button type="submit" disabled={!!carError || !selectedCarId} className="w-full xl:w-auto flex-[2] bg-indigo-600 text-white py-4 rounded-3xl font-black uppercase tracking-widest hover:bg-indigo-700 shadow-2xl active:scale-95 flex items-center justify-center gap-3 transition-transform">
                            <Plus size={24}/> {editingBookingId ? 'Simpan Perubahan' : 'Simpan Transaksi'}
                        </button>
                    </div>
                </div>
             </form>
