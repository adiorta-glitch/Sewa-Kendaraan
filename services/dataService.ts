...
export const INITIAL_CARS: Car[] = [
  { 
    id: 'c1', name: 'Toyota Avanza', plate: 'B 1234 ABC', type: 'MPV', 
    pricing: { '12 Jam (Dalam Kota)': 300000, '24 Jam (Dalam Kota)': 450000, 'Full Day (Luar Kota)': 550000 },
    status: 'Available', image: 'https://picsum.photos/300/200?random=1' 
  },
  { 
    id: 'c2', name: 'Honda Brio', plate: 'B 5678 DEF', type: 'City Car', 
    pricing: { '12 Jam (Dalam Kota)': 250000, '24 Jam (Dalam Kota)': 350000, 'Full Day (Luar Kota)': 450000 }, 
    status: 'Available', image: 'https://picsum.photos/300/200?random=2' 
  },
];
...
export const DEFAULT_SETTINGS: AppSettings = {
  companyName: 'Bersama Rent Car', tagline: 'Solusi Perjalanan Anda', address: 'Jl. Raya Utama No. 88, Jakarta',
  phone: '0812-3456-7890', email: 'admin@bersamarentcar.com', website: 'www.bersamarentcar.com',
  invoiceFooter: 'Terima kasih telah menyewa di Bersama Rent Car.', logoUrl: null, themeColor: 'red', darkMode: false,
...
// ==========================================
// 3. FUNGSI HELPER (Pricing, Availability, CSV)
// ==========================================
...
export const checkAvailability = (carId: string, startDate: string, endDate: string, bookings: Booking[]): boolean => {
  ...
  return !bookings.some((booking) => {
    if (booking.carId !== carId || booking.status === 'Cancelled' || booking.status === 'Completed') return false;
...
export const calculatePricing = (car: Car, rentalPackage: string, duration: number, highSeasons: HighSeason[], startDate: string): number => {
  if (!car || !car.pricing) return 0;
  let multiplier = rentalPackage.includes('12 Jam') ? 1 : duration;
  let totalPrice = (car.pricing[rentalPackage] || 0) * multiplier;
  ...
  if (activeSeason) totalPrice += (activeSeason.priceIncrease * multiplier);
  return totalPrice;
};
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      const result = lines.slice(1).filter(l => l.trim()).map(line => {
        const values = line.split(','); const obj: any = {};
        headers.forEach((h, i) => {
          let val = values[i]?.trim().replace(/^"|"$/g, '');
          if (val === 'true') obj[h] = true; else if (val === 'false') obj[h] = false; else obj[h] = val;
        });
        return obj;
      });
      resolve(result);
...
export const mergeData = (existingData: any[], newData: any[]) => {
    ...
    newData.forEach(item => {
        const id = item.id || 'imported_' + Math.random().toString(36).substr(2, 9);
        dataMap.set(id, { id, ...item, ...dataMap.get(id) });
    });
